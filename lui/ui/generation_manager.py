...
class GenerationManager:
    ...
    def build_full_prompt(self) -> str:
        """Build the full prompt including all completion history"""
        if not self.completion_history:
            # First generation - use original prompt
            return self.app.query_one("#prompt-input").value
        else:
            # Subsequent generations - concatenate all previous completions
            full_sequence = self.original_prompt
            for entry in self.completion_history:
                full_sequence += entry['result']
    ...
    async def handle_countdown(self, chosen_idx: int, chosen_score: int) -> bool:
        ...
        if prompt_input:
            ...
            self.completion_history.append({
                'prompt': self.build_full_prompt(),  # Record the full prompt used for this generation
                'result': chosen_completion,
                'score': chosen_score
            })
            # Update the input to show the new full sequence for user reference
            new_full_sequence = self.original_prompt
            for entry in self.completion_history:
                new_full_sequence += entry['result']
            prompt_input.value = new_full_sequence
            self.app.show_input_view()
            return True
        return False
